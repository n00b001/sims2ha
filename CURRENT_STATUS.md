...
The `npm run lint` command in `package.json` calls `eslint src/sims2-icons.js src/sims2-loading-card.js src/sims2-gauge-card.js src/sims2-divider-card.js 2>&1 || true`. The `|| true` at the end means CI *never* fails on JS lint errors. We should update this to remove the `|| true` now that eslint has real rules.
