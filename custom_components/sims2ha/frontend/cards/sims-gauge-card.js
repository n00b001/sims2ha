...
class SimsGaugeCard extends LitElement {
  ...
  setConfig(config) {
    ...
    this.showDivider = config.showDivider !== undefined ? config.showDivider : true;
  ...
  render() {
    if (this._state === null) {
      return html`<div class="need-row"><div class="need-icon">?</div><div class="need-bar"></div></div>`;
    }

    const percent = Math.max(0, Math.min(100, ((this._state - this.min) / (this.max - this.min)) * 100));
    let colorClass = 'green';
    if (percent >= 80) {
      colorClass = percent < 30 ? 'red' : (percent < 50 ? 'orange' : (percent < 70 ? 'yellow' : 'green'));
    }
    ...
    return html`
      <div class="need-row">
        ...
        <div class="need-bar ${this.size} ${this.glowIntensity}" style="--gradient: ${gradient};">
  ...
  getGradientForColor(color) {
    switch (color) {
      case 'green': return 'linear-gradient(90deg, #4E9A26, #7BC942)';
      case 'red': return 'linear-gradient(90deg, #CC0000, #E55B45)';
      case 'yellow': return 'linear-gradient(90deg, #F4A460, #F2C14E)';
      case 'blue': return 'linear-gradient(90deg, #4A90E2, #2F86C5)';
      default: return 'linear-gradient(90deg, #4E9A26, #7BC942)';
    }
  }
